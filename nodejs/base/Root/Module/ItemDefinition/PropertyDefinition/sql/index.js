 ...
 * included in it
 */
function convertGQLValueToSQLValueForProperty(knex, serverData, mod, itemDefinition, include, propertyDefinition, data, oldData, uploadsClient, domain, dictionary) {
    ...
    if (description.gqlAddFileToFields) {
        ...
        if (description.gqlList) {
            const processedValue = file_management_1.processFileListFor(newValue, oldValue, uploadsClient, domain, itemDefinition || mod, include, propertyDefinition);
            gqlPropertyValue = processedValue.value;
            consumeStreams = processedValue.consumeStreams;
        }
        else {
            const processedValue = file_management_1.processSingleFileFor(newValue, oldValue, uploadsClient, domain, itemDefinition || mod, include, propertyDefinition);
            gqlPropertyValue = processedValue.value;
            consumeStreams = processedValue.consumeStreams;
        }
    }
