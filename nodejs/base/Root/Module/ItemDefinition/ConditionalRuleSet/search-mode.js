 ...
 */
function getConversionRulesetId(rawData) {
    // we get all the possible ids
    const ids = search_mode_1.getConversionIds(rawData);
    // and return such, or null if we got an empty array
    // (this happens when search mode is disabled for that property)
    return ids.length ? ids[0] : null;
}
exports.getConversionRulesetId = getConversionRulesetId;
/**
 ...
 */
function buildSearchModeConditionalRuleSet(rawData, otherKnownProperties) {
    // we make a copy
    const newRule = { ...rawData };
    // if it's a property based rule
    if (newRule.property) {
        // we get the rule as a property
        const newRuleAsProperty = newRule;
        ...
        if (newRuleAsProperty.property !== "&this") {
            ...
            const converted = getConversionRulesetId(otherKnownProperties[newRuleAsProperty.property]);
        ...
        const valueAsAlternative = newRuleAsProperty.value;
        // if that's the case
        if (valueAsAlternative && valueAsAlternative.property) {
            // we need to convert that one too!
            newRuleAsProperty.value = {
                property: getConversionRulesetId(otherKnownProperties[valueAsAlternative.property]),
            };
...
exports.buildSearchModeConditionalRuleSet = buildSearchModeConditionalRuleSet;
