...
class ActualSearchLoader extends react_1.default.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        const currentSearchResults = this.props.searchResults.slice(this.props.pageSize * this.props.currentPage, this.props.pageSize * (this.props.currentPage + 1));
        ...
        if (prevProps.searchId !== this.props.searchId ||
            !deep_equal_1.default(this.state.currentSearchResults, currentSearchResults)) {
    ...
    async loadValues(currentSearchResults) {
        ...
        const workerCachedResults = await Promise.all(currentSearchResults.map(async (searchResult) => {
    ...
    render() {
        ...
        return (<item_definition_1.SearchItemDefinitionValueContext.Provider value={{
        ...
        {this.props.children({
            searchResults: this.state.error ? [] : this.state.currentSearchResults.map((searchResult) => {
                const itemDefinition = this.props.itemDefinitionInstance
                    .getParentModule().getParentRoot().registry[searchResult.type];
                return {
                    ...searchResult,
                    providerProps: {
...
class PagedSearchLoader extends react_1.default.Component {
    ...
    render() {
        return (<SearchLoader pageSize={this.props.pageSize} currentPage={this.state.currentPage}>
        {(arg) => {
