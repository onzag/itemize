...
class PropertyEntryField extends react_1.default.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        return nextProps.property !== this.props.property ||
            !deep_equal_1.default(this.state.suggestions, nextState.suggestions) ||
    ...
    renderBasicTextField(textFieldProps) {
        ...
        if (type === "password") {
          ...
          <core_1.IconButton classes={{ root: this.props.classes.iconButton }} onClick={this.toggleVisible}>
            <core_1.Icon>
        ...
        <TextField_1.default fullWidth={true} type={this.state.visible ? "text" : "password"} className={this.props.classes.entry} label={i18nLabel} placeholder={i18nPlaceholder} value={this.props.state.internalValue || this.props.state.value || ""} onChange={this.onChangeByHTMLEvent} InputProps={{
            classes: {
        ...
        }} inputProps={inputProps} disabled={this.props.state.enforced} variant="filled" {...appliedTextFieldProps}/>
    ...
    renderAutosuggestSuggestion(suggestion, params) {
        ...
        const valueToMatch = this.props.property.isAutocompleteLocalized() ?
            suggestion.i18n :
            suggestion.value;
        const matches = match_1.default(valueToMatch, params.query);
        const parts = parse_1.default(valueToMatch, matches);
        return (<core_1.MenuItem className={this.props.classes.autocompleteMenuItem} selected={params.isHighlighted} component="div">
        <div className={this.props.classes.autocompleteMenuItemMainText}>
          {parts.map((part, index) => part.highlight ? (<span key={index} style={{ fontWeight: 500 }}>
                  {part.text}
                </span>) : (<strong key={index} style={{ fontWeight: 300 }}>
                  {part.text}
                </strong>))}
        </div>
      </core_1.MenuItem>);
    ...
    async onSuggestionsFetchRequested({ value }) {
        ...
        try {
            const result = await fetch("/rest/autocomplete/" + this.props.property.getAutocompleteId() +
                "?body=" + encodeURIComponent(JSON.stringify({
                lang: this.props.property.isAutocompleteLocalized() ? this.props.language : null,
                query: value,
                filters: this.props.property.getAutocompletePopulatedFiltersFor(this.props.forId, this.props.forVersion),
            })), {
                headers: {
                    "sw-cacheable": "true",
                },
