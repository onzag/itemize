...
class RemoteListener {
    ...
    addOwnedSearchListenerFor(itemDefinitionOrModuleQualifiedPathName, createdBy, lastKnownRecord, callback) {
        if (!this.isReady) {
            throw new Error("Remote listener is not ready");
        }
    ...
    addParentedSearchListenerFor(itemDefinitionOrModuleQualifiedPathName, parentType, parentId, parentVersion, lastKnownRecord, callback) {
        if (!this.isReady) {
            throw new Error("Remote listener is not ready");
        }
    ...
    async onRecordsAddedToOwnedSearch(event) {
        ...
        if (ownedListener) {
            ...
            if (cache_1.default.isSupported) {
                ...
                await cache_1.default.instance.addRecordsToCachedSearch(constants_1.PREFIX_SEARCH + itemDefinition.getSearchModeCounterpart().getQualifiedPathName(), event.createdBy, null, null, null, event.newIds, event.newLastRecord, "by-owner");
            }
    ...
    async onRecordsAddedToParentedSearch(event) {
        ...
        if (parentedListener) {
            ...
            if (cache_1.default.isSupported) {
                ...
                await cache_1.default.instance.addRecordsToCachedSearch(constants_1.PREFIX_SEARCH + itemDefinition.getSearchModeCounterpart().getQualifiedPathName(), null, event.parentType, event.parentId, event.parentVersion, event.newIds, event.newLastRecord, "by-parent");
            }
