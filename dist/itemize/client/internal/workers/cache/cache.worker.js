...
class CacheWorker {
    ...
    setupVersion(version) {
        ...
        this.dbPromise = idb_1.openDB(exports.CACHE_NAME, version, {
            upgrade(db) {
                try {
    ...
    async setCachedValueAsNullAndUpdateSearches(id, version, type, queryName, searchQueryNameModule, searchQueryNameItemDefinition) {
        ...
        try {
            const allKeys = await db.getAllKeys(exports.SEARCHES_TABLE_NAME);
            await Promise.all(allKeys.map(async (key) => {
                try {
                    ...
                    if (!searchQueryNameModule ||
                        ...
                        const currentValue = await db.get(exports.SEARCHES_TABLE_NAME, key);
                        const foundIndex = currentValue.value.findIndex((v) => v.id === id && v.type === type);
                        if (foundIndex !== -1) {
                            currentValue.value.splice(foundIndex, 1);
                            await db.put(exports.SEARCHES_TABLE_NAME, currentValue, key);
                        }
                    }
                }
                catch (err) {
    ...
    async setCachedValue(queryName, id, version, partialValue, partialFields, touchOrMerge) {
        ...
        try {
            ...
            await db.put(exports.QUERIES_TABLE_NAME, idbNewValue, queryIdentifier);
        }
        catch (err) {
            console.warn(err);
            return false;
    ...
    async deleteCachedValue(queryName, id, version) {
        ...
        const queryIdentifier = `${queryName}.${id}.${(version || "")}`;
        try {
            await db.delete(exports.QUERIES_TABLE_NAME, queryIdentifier);
        }
        catch (err) {
            console.warn(err);
            return false;
    ...
    async getCachedValue(queryName, id, version, requestedFields) {
        ...
        try {
            ...
            const idbValue = await db.get(exports.QUERIES_TABLE_NAME, queryIdentifier);
    ...
    async addRecordsToCachedSearch(searchQueryName, createdByIfKnown, parentTypeIfKnown, parentIdIfKnown, parentVersionIfKnown, newIds, newLastRecord, cachePolicy) {
        // so we fetch our db like usual
        let db;
        try {
            db = await this.dbPromise;
        ...
        try {
            const currentValue = await db.get(exports.SEARCHES_TABLE_NAME, storeKeyName);
            await db.put(exports.SEARCHES_TABLE_NAME, {
                ...currentValue,
    ...
    async runCachedSearch(searchQueryName, searchArgs, getListQueryName, getListTokenArgs, getListLangArgs, getListRequestedFields, cachePolicy) {
        ...
        try {
            ...
            const dbValue = await db.get(exports.SEARCHES_TABLE_NAME, storeKeyName);
            ...
            if (!dbValue) {
                ...
                const actualArgsToUseInGQLSearch = {
                    ...
                    order_by: new gql_querier_1.GQLEnum("DEFAULT"),
                };
                if (cachePolicy === "by-owner") {
                ...
                if (gql_util_1.requestFieldsAreContained(getListRequestedFields, dbValue.fields) &&
                    ...
                    const gqlValue = {
                        data: {
                            [searchQueryName]: {
                                ids: await cache_worker_search_1.search(this.rootProxy, db, resultsToProcess, searchArgs),
        ...
        uncachedResultsToProcess.forEach((uncachedResultToProcess) => {
            ...
            if (batches[lastBatchIndex].length === constants_1.MAX_SEARCH_RESULTS_AT_ONCE_LIMIT) {
                batches.push([]);
                lastBatchIndex++;
            }
        ...
        if (!somethingFailed) {
            ...
            // 1,2,3,4,5
            const actualCurrentSearchValue = await db.get(exports.SEARCHES_TABLE_NAME, storeKeyName);
            await db.put(exports.SEARCHES_TABLE_NAME, {
            ...
            const gqlValue = {
                data: {
                    [searchQueryName]: {
                        ids: await cache_worker_search_1.search(this.rootProxy, db, actualCurrentSearchValue.value, searchArgs),
                    },
                },
            };
            return {
