...
function checkPropertyDefinition(rawData, parentItemDefinition, parentModule, traceback) {
    ...
    if (!propertyDefintionTypeStandard.supportsAutocomplete && rawData.autocomplete) {
        throw new Error_1.default("Cannot use autocomplete on property type '" + rawData.type + "'", traceback.newTraceToBit("autocomplete"));
    }
    if (rawData.values && rawData.autocomplete) {
        throw new Error_1.default("Cannot use autocomplete and values at the same time", traceback.newTraceToBit("autocomplete"));
    }
    // Let's check whether the autocomplete properties are there
    if (rawData.autocompleteFilterFromProperty) {
        const autocompleteFilterFromPropertyTraceback = traceback.newTraceToBit("autocompleteFilterFromProperty");
        Object.keys(rawData.autocompleteFilterFromProperty).forEach((keyId) => {
            ...
            if (!ItemDefinition_1.default.getPropertyDefinitionRawFor(parentItemDefinition, parentModule, propertyId, true)) {
                throw new Error_1.default("Invalid autocomplete property to funnel", autocompleteFilterFromPropertyTraceback.newTraceToBit(keyId));
            }
        });
    }
 ...
 */
function checkAutocompleteFilterAndValues(rawDataConfig, rawData, traceback) {
    ...
    rawData.forEach((value, index) => {
        ...
        if (value.type === "filter") {
            ...
            // stacking filters
            if (value.values) {
                checkAutocompleteFilterAndValues(rawDataConfig, value.values, traceback.newTraceToBit(index).newTraceToBit("values"));
            }
            // if we have filters within itelf, which contain itself values
            if (value.filters) {
                // we check those as well
                checkAutocompleteFilterAndValues(rawDataConfig, value.filters, traceback.newTraceToBit(index).newTraceToBit("filters"));
            }
        }
        else {
            ...
            if (value.i18n) {
                ...
                rawDataConfig.supportedLanguages.forEach((language) => {
                    ...
                    if (!value.i18n[language]) {
                        // throw an error
                        throw new Error_1.default("Missing locale value for language " + language, i18nTraceback);
                    }
                });
            }
        }
...
exports.checkAutocompleteFilterAndValues = checkAutocompleteFilterAndValues;
