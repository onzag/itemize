 ...
 */
async function getActualFileLocation(partialLocation, traceback) {
    let actualFileLocation = partialLocation;
    // check if exists without throwing an error
    const exists = await checkExists(partialLocation);
    // if it exists we must check if it's the right type
    if (exists) {
        // it must be a directory
        const isDirectory = await checkIsDirectory(partialLocation, traceback);
        // if it's a directory as it should add the index.json
        if (isDirectory) {
            actualFileLocation = path_1.default.join(partialLocation, "index.json");
        }
    }
    ...
    await checkExists(actualFileLocation, traceback);
